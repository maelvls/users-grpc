...
func getDB() *pg.DB {
	...
	log.Printf("unit tests will use postgres at: %v", db)
	err := InitSchema(db)
	if err != nil {
		panic(err)
	}
...
func initDBWith(users []*pb.User) func(*pg.Tx) {
	return func(tx *pg.Tx) {
		for _, user := range users {
			if _, err := tx.Model(user).OnConflict("(id) DO UPDATE").Insert(); err != nil {
				panic(fmt.Errorf("inserting user %s: %w", user.Name, err))
			}
		}
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := db.RunInTransaction(func(tx *pg.Tx) error {
				...
				tt.given(tx)
				args := tt.when(tx)
				got, err := svc.Create(args.ctx, args.req)
				if td.CmpNoError(t, err) && tt.then != nil {
					tt.then(t, tx, got)
				}
				return nil
			})
